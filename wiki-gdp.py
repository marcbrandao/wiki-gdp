                ...
                if gdp_col_index is not None:
                    ...
                    for row in rows[1:]:  # Skip the header row
                        ...
                        if len(cells) > gdp_col_index:  # In case there are more than the index column, the first column should be the year
                            year = cells[0].text[
                                   :4]  # Only grab first 4 characters on the Year collumn (skipping notes)
...
# Checking if its being ran in Jupyter or Python standard:


# Check if running in a Jupyter notebook
if get_ipython() is not None and 'IPKernelApp' in get_ipython().config:
    # Running in Jupyter notebook, use display
